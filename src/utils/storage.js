...
export const wipeAllData = async () => {
  try {
    ...
    if (session && supabase) {
      ...
      const { error: storiesError } = await supabase.from('stories').delete().eq('user_id', userId);

      if (storiesError) {
        console.warn('Failed to delete stories from cloud:', storiesError);
      }
      ...
      if (progressError) {
        console.warn('Failed to delete progress from cloud:', progressError);
      }
      ...
      if (!listError && files && files.length > 0) {
        ...
        const { error: deleteError } = await supabase.storage.from('image-cache').remove(filePaths);

        if (deleteError) {
          console.warn('Failed to delete some images from cloud:', deleteError);
        }
      }
    }
...
export const syncAll = async () => {
  ...
  try {
    // 1. Sync Stories
    const { data: remoteStories } = await supabase.from('stories').select('*');
    ...
    if (remoteProgress) {
      ...
      remoteProgress.forEach(rp => {
        if (
          ...
          rp.updated_at > new Date(localProgress[rp.story_id].lastRead).toISOString()
        ) {
          localProgress[rp.story_id] = {
